...
//! * `try_reserve` never panics; `Full` is the only acceptable error
//!   alongside `AlreadyPresent`.
//! * After `drain()`, `len() == 0` and the table is reusable.
//!
//! Op encoding (5 bytes each):
//!   tag(1) + pid(4)
//!   tag 0 → try_reserve(pid), then commit_reserved(pid_as_value)
//!   tag 1 → remove(pid)
//!   tag 2 → contains(pid)
//!   tag 3 → drain (full)
    ...
    while off + 5 <= data.len() {
        ...
        match chunk[0] & 0x03 {
            0 => {
                ...
                match table.try_reserve(pid) {
                    Ok(reservation) => {
                        assert_eq!(table.len(), pre_len + 1);
                        assert!(table.len() <= capacity);
                        assert_eq!(table.get(pid), None);
                        table.commit_reserved(reservation, pid);
                        assert_eq!(table.get(pid), Some(&pid));
                    }
