# bug-433 — NetBSD `SCM_CREDS` / `struct sockcred` ABI fix (SHIPPED — field validation pending)

**Status:** IMPLEMENTED in `crates/varta-watch/src/peer_cred/platform/bsd.rs` with fabricated-buffer coverage and a NetBSD cross-compile check. Remaining work: run a live NetBSD host sign-off and record one accepted beat from real kernel-delivered credentials.
**Severity:** HIGH (total silent denial of stall-detection/recovery on a declared-supported platform).
**Target files:** `crates/varta-watch/src/peer_cred/platform/bsd.rs`, `crates/varta-watch/src/peer_cred/cmsg.rs`, `.github/workflows/ci.yml`, `book/src/architecture/peer-authentication.md`
...
Pre-fix `bsd.rs` grouped FreeBSD / DragonFly / NetBSD and unconditionally hardcoded the **FreeBSD** credential ABI:

| Symbol (pre-fix code) | Value | Correct on NetBSD? |
|---|---|---|
...
4. CI did not catch it: `cross-compile-checks` compiled only illumos + OpenBSD + riscv64 + musl; the sole BSD walker test fabricated a **FreeBSD** `0x03`/`cmsgcred` buffer, baking in the wrong assumption.

This was **two** independent defects: the wrong `LOCAL_CREDS` value, and the wrong `SCM_CREDS`+struct. The `LOCAL_CREDS` value was wrong even for the in-tree NetBSD `cfg` arm that already existed.
...
**`LOCAL_OCREDS = 0x0001`** is what the pre-fix code mistakenly used for NetBSD `LOCAL_CREDS`. Confirmed in NetBSD `un.h`: `#define LOCAL_OCREDS 0x0001`.
...
**Doc fix shipped:** bsd.rs now labels `cmsgcred` **FreeBSD/DragonFly-only**.

---

## (b) Decision: cfg-split FFI vs route-NetBSD-to-`SocketModeOnly`

**Decision shipped: cfg-split FFI per target.** FreeBSD/DragonFly keep `Cmsgcred` + `SCM_CREDS=0x03` + `LOCAL_CREDS=0x0002`; NetBSD gets `Sockcred` + `SCM_CREDS=0x10` + `LOCAL_CREDS=0x0004` behind a `NetBsdCmsg` `CmsgPlatform` impl.
...
**Staging status:**
1. Done: cfg-split FFI (`NetBsdCmsg` + `Sockcred` + correct constants) guarded by compile-time `size_of` / field-offset assertions.
2. Done: Linux-host fabricated-buffer test proving the NetBSD walker arm reads `sc_pid@0` / `sc_euid@8` from a NetBSD-shaped buffer and rejects FreeBSD's `SCM_CREDS=0x03`.
3. Done: `book/src/architecture/peer-authentication.md` marks NetBSD as ABI-verified but not field-validated.
4. Pending: empirical NetBSD host sign-off.

**Not recommended:** `SocketModeOnly` interim — it is a real, permanent regression given per-datagram PID is verified available. It is the correct fallback **only** if a maintainer explicitly vetoes minting `KernelAttested` from a not-yet-field-validated struct; in that case `cfg` NetBSD onto the existing OpenBSD plain-recv path with a `// TODO bug-433` marker.

---

## (c) CI

### 1. NetBSD cross-compile job (`cross-compile-checks`, ci.yml)

NetBSD is Tier-2 with no pre-built std on the stable channel → follow the **OpenBSD pattern** (nightly + `-Zbuild-std`). The CI job switches to nightly + `rust-src` for OpenBSD and NetBSD:
...
The job name includes "+ NetBSD" so the target stays visible in the Actions UI.

### 2. NetBSD-shaped fabricated-buffer unit test (mirror the FreeBSD walker test)

`mod bsd` is compiled on Linux (platform/mod.rs:30-36) so `NetBsdCmsg`/`Sockcred` stay Linux-visible for tests while the FFI extern block / `use` remains target-gated. The Linux-host test sits alongside the FreeBSD walker test, deliberately parallel but with NetBSD's `(SOL_SOCKET, 0x10)` pair and the `sockcred` offsets:
...
4. **Live-runner gap** — no NetBSD GHA runner; the CI job is type-check-only; no live `varta-watch → /metrics` interop test can run.

**Current posture:** the cfg-split FFI ships with compile-time layout guards, Linux-host fabricated-buffer coverage, a NetBSD type-check CI lane, and peer-authentication docs that mark NetBSD **not field-validated**. Keep bug-433 OPEN only for a maintainer to run one live beat on a NetBSD host. Do **not** route NetBSD to `SocketModeOnly` (verified capability regression) unless a maintainer vetoes provisional `KernelAttested`.

**Net:** evidence was sufficient to ship the cfg-split safely behind compile-time + miri guards (constants and front-field offsets are HIGH/MEDIUM-HIGH; the realistic failure mode is statically caught). It is NOT sufficient to declare NetBSD field-validated — that requires a NetBSD host and remains a tracked, open follow-up.
