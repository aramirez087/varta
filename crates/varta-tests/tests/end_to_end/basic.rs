...
use super::{
    http_get, skip_if_uds_recovery_unsupported, spawn_watch, wait_until, ChildGuard, TempDir,
    AGENT_CHILD_ENV, DEGRADED_CHILD_ENV, MULTI_AGENT_BEATS, MULTI_AGENT_COUNT, PANIC_CHILD_ENV,
};
...
pub(super) fn client_to_observer_to_recovery_full_loop() {
    if skip_if_uds_recovery_unsupported("client_to_observer_to_recovery_full_loop") {
        return;
    }
