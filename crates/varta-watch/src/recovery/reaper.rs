...
/// never trips, so [`Self::capture_ready_to_complete`] would otherwise never
/// return true.
/// Past this window the slot is reclaimed with whatever was captured and the
/// completion audit record is marked `truncated`; capture is best-effort and
/// bounded, so dropping a wedged tail is preferable to an unbounded leak that
/// permanently disables recovery for the pid.
...
impl Recovery {
    ...
    pub(super) fn reap_finished_child(
        ...
        let step = {
            ...
            } else if Self::capture_ready_to_complete(entry_mut) {
    ...
    pub(super) fn capture_ready_to_complete(entry: &mut Outstanding) -> bool {
        if matches!(entry.completed_at, Some(t) if t.elapsed() >= POST_EXIT_CAPTURE_DRAIN_GRACE) {
            if entry.stdout_handle.is_some() || entry.stderr_handle.is_some() {
