...
//! kernel-supplied ancillary buffer produced by `recvmsg(2)`. macOS uses
//! `getsockopt(LOCAL_PEERTOKEN)` instead and is therefore not represented
//! here.
//!
//! At this commit the Linux and BSD walkers are *duplicated* (4 unsafe
//! functions each, differing only in the integer width of `cmsg_len` and
//! the target `(level, type)` pair). The next commit unifies them behind
//! an `unsafe trait CmsgPlatform`.
...
pub(super) const fn cmsg_hdr_size() -> usize {
...
#[cfg(target_os = "linux")]
pub(super) unsafe fn cmsg_firsthdr(mhdr: &plat::Msghdr) -> Option<&plat::Cmsghdr> {
...
#[cfg(target_os = "linux")]
pub(super) unsafe fn find_credential_pid(
