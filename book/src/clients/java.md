...
| --------- | ------ | ----- |
| Unix Domain Sockets | Supported (Linux, macOS) | `Varta.connect(path)`. Requires a UDS provider on the classpath (`junixsocket` recommended). The only transport classified `BeatOrigin::KernelAttested`, so it is the only JVM transport eligible for observer-driven recovery. `NoUdsTransportException` on Windows. |
